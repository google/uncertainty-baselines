...
r"""Rank1 BNN baseline for Diabetic Retinopathy Detection, evaluated on the OOD APTOS Indian retinopathy dataset with moderate decision threshold.
"""
...
def get_config():
  ...
  output_dir = 'gs://drd-rank1-aptos-results/{}'.format(config.experiment_name)
  config.args = {
      'batch_size': 16,
...
def get_sweep(hyper):
  ...
  return hyper.zipit([
      ...
      hyper.uniform('fast_weight_lr_multiplier', hyper.interval(0.5, 1.25))
  ],
                     length=num_trials)
