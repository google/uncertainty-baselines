...
class Loss:
  def __init__(
    ...
    Args:
      ...
      n_samples: the number of Monte-Carlo samples used to estimate the posterior.
  ...
  def _crossentropy_log_likelihood(
    self, preds_f_samples: jnp.ndarray, targets: jnp.ndarray
  ) -> jnp.ndarray:
  ...
  def _function_kl(
    ...
    Args:
      ...
      inducing_inputs: used for computing scale and function distribution used in KL
  ...
  def crossentropy_log_likelihood(
    self,
  ...
  def nelbo_fsvi_classification(
    ...
    Args:
      ...
      targets: labels.
      inducing_inputs: inducing inputs on which to compute the posterior distribution.
      rng_key: jax random key.
      class_weight: if True, address class imbalance by using different weights for each class in the
        log likelihood term.
    ...
    Returns:
      ...
      dict, a dictionary containing the constituents of the ELBO loss for debugging
    """
    is_training = True
...
@jit
def partition_params(params: hk.Params) -> Tuple[hk.Params, hk.Params, hk.Params]:
