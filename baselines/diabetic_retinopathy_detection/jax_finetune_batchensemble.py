...
def main(_):
  ...
  # Unpack model init and random sign init
  config.model_init = config.model_init_and_random_sign_init[0]
  config.model.transformer.random_sign_init = (
    config.model_init_and_random_sign_init[1])
  ...
  # train_loss = -jnp.inf
  # eval_loss = {eval_name: -jnp.inf for eval_name, _ in eval_iter_splits.items()}
  ...
  for step, train_batch, lr_repl in zip(
    ...
    if config.get('only_eval', False) or train_utils.itstime(
      ...
      vit_utils.write_note('Evaluating on the evalidation sets...')
      chrono.pause()
      all_eval_results = {}
