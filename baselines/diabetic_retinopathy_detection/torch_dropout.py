...
def main(argv):
  ...
  logging.info(
    'Model number of weights: %s', torch_utils.count_parameters(model))

  # Linearly scale learning rate and the decay epochs by vanilla settings.
  base_lr = FLAGS.base_learning_rate
  optimizer = torch.optim.SGD(
    model.parameters(), lr=base_lr, momentum=1.0 - FLAGS.one_minus_momentum,
    nesterov=True)
  ...
  def run_train_epoch(iterator):
    def train_step(inputs):
      ...
      images = torch.from_numpy(images._numpy()).view(
        train_batch_size, 3, image_H, image_W).to(device)
    ...
    for step in range(steps_per_epoch):
      ...
      if step % 100 == 0:
        ...
        eta_seconds = (
          max_steps - current_step) / steps_per_sec if steps_per_sec else 0
  ...
  def run_eval_epoch(iterator, dataset_split, num_steps):
    def eval_step(inputs, model):
      ...
      labels = inputs['labels']
      images = torch.from_numpy(images._numpy()).view(
        eval_batch_size, 3, image_H, image_W).to(device)
      labels = torch.from_numpy(
        labels._numpy()).to(device).float().unsqueeze(-1)
  ...
  final_checkpoint_path = os.path.join(
    FLAGS.output_dir, f'model_{FLAGS.train_epochs}.pt')
  torch_utils.checkpoint_torch_model(
    model=model, optimizer=optimizer, epoch=FLAGS.train_epochs,
