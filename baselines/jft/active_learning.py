...
flags.DEFINE_enum("acquisition_method", default="uniform",
...
def get_ids_logits_masks(*,
  ...
  for batch in iter_ds:
    ...
    batch_output = compute_batch_outputs(opt_repl.target,
                                          batch["image"])
...
def get_density_scores(*,
  ...
  scores[pool_masks_bool] = scores[pool_masks_bool].max() - \
...
def finetune(*,
  ...
  best_opt_accuracy = -1
  best_step = 1
  for current_step, train_batch, lr_repl in zip(tqdm.trange(1, total_steps + 1),
                                                iter_ds,
                                                lr_iter):
    ...
    if current_step % 5 == 0:
      train_accuracy = get_accuracy(evaluation_fn=evaluation_fn,
                                    opt_repl=opt_repl,
                                    ds=train_eval_ds)
      val_accuracy = get_accuracy(evaluation_fn=evaluation_fn,
                                  opt_repl=opt_repl,
                                  ds=val_ds)
