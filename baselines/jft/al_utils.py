...
class TFDSIdToInt:
  ...
  def __init__(self, dataset_info: tfds.core.DatasetInfo, *,
    ...
    # Build a unique offset across splits or within splits depending on `splitwise_id`.
...
class SubsetDatasetBuilder(DatasetBuilder):
  """Subset Dataset Builder which is "just right" for clu.
    ...
    Args:
      base_dataset_builder: a DatasetBuilder for the underlying dataset (essentially an object with a as_dataset method)
  ...
  def as_dataset(self,
    ...
    # We don't allow an empty split by virtue of the parameter declaration, so we always have a split.
    ...
    # This is a bit more complex: potentially cache before or after calling .shuffle.
    # BUT don't cache for the pool set as it will be much larger than the training set.
    reshuffle_each_iteration = shuffle_files and read_config.shuffle_reshuffle_each_iteration is not False
    ...
    if shuffle_files:
      if self.subset_ids is not None:
        ...
        # TODO: what buffer size do we want actually for shuffling? 10k seems like a safe thing.
        buffer_size = 10000
