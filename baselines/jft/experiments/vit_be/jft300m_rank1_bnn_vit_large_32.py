...
r"""Rank-1 BNN ViT L/32 on JFT-300M.
...
def get_config():
  ...
  # Rank-1 BNN
  config.prior_mean = 1.
  config.prior_std = 0.05  # Set in sweep.
  config.eval_samples = 4  # Set in sweep.
  ...
  return hyper.product([
      hyper.chainit([
          ...
          hyper.product([
              ...
              hyper.sweep('config.model.transformer.be_layers', [(21, 22, 23),
                                                                 (22, 23)]),
          ]),
      ]),
      ...
      hyper.sweep('config.prior_std', [0.05, 0.1]),
      hyper.sweep('config.eval_samples', [1, 4]),
  ])
