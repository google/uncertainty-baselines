...
def main(config, output_dir):
  ...
  def update_fn(opt, rngs, lr, images, labels):
    rngs, next_rngs = batchensemble_utils.tree_rngs_split(rngs, 2)
    rngs, rng_model = jax.random.split(rngs, 2)
    ...
    if max_grad_norm_global and max_grad_norm_global > 0.0:
      ...
      grads, global_norm = batchensemble_utils.tree_clip_norm_global_pmax(
    ...
    if fast_weight_lr_multiplier and fast_weight_lr_multiplier != 1.0:
      ...
      match_fn = lambda name: ('fast_weight_alpha' in name  # pylint: disable=g-long-lambda
                               or 'fast_weight_gamma' in name)
      grads = batchensemble_utils.tree_map_with_names(
          fast_weights_lr_fn, grads, match_fn)
