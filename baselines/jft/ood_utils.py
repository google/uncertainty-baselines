...
def compute_mean_and_cov(
    embeds: jnp.ndarray, labels: jnp.ndarray,
    class_ids: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray]:
  ...
  Returns:
    A tuple of (means, cov), where `means` is an jnp.array of size [num_classes,
...
@jax.jit
def compute_mahalanobis_distance(embeds: jnp.ndarray, means: jnp.ndarray,
  ...
  def maha_dist(x, mean):
    diff = x - mean
    return jnp.einsum('i,ij,j->', diff, cov_inv, diff)  # Squared Maha distance.
  ...
  for ood_ds_name in ood_ds_names:
        ...
        if means is not None and cov is not None:
          if not use_ens:
            ...
            for m in range(ens_size):
              dists = compute_mahalanobis_distance(embeds[..., m], means[m],
                                                   cov[m])
    ...
    if ood_ds_name == 'train_maha':
      ...
      if not use_ens:
        ...
        means, cov = compute_mean_and_cov(pre_logits_train, labels_train,
                                          class_ids)
