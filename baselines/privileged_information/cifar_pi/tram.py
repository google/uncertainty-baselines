...
"""Wide ResNet 28-10 with TRAM head trained on CIFAR10/100-N.
...
flags.DEFINE_bool('use_heteroscedastic', False,
                  'Whether or not to use the heteroscedastic layer.')
...
flags.DEFINE_integer(
    'het_num_mc_samples', 10000,
    'Number of MC samples to use in the heteroscedastic layer.'
)
# OOD flags.
...
flags.DEFINE_string(
    ...
    'Comma-separated string defining subset of additional PI.'
)
...
flags.DEFINE_float('no_pi_loss_weight', 1.,
...
def main(argv):
  ...
  with strategy.scope():
    if FLAGS.use_heteroscedastic:
      logging.info('Building ResNet Het-TRAM model.')
      model = ub.models.wide_resnet_het_tram(  # type: ignore
          input_shape=(32, 32, 3),
      ...
      logging.info('Building ResNet TRAM model.')
      model = ub.models.wide_resnet_tram(  # type: ignore
          input_shape=(32, 32, 3),
          depth=FLAGS.model_depth,
          width_multiplier=FLAGS.model_width_multiplier,
          num_classes=num_classes,
  ...
  def train_step(iterator):
    ...
    def step_fn(inputs):
      ...
      with tf.GradientTape() as tape:
        ...
        loss = (
            FLAGS.no_pi_loss_weight * negative_log_likelihood
            + negative_log_likelihood_pi
            + l2_loss
        )
  ...
  def validation_step(iterator, num_steps, use_annotator_labels):
    ...
    def step_fn(inputs):
      ...
      metrics[f'{validation_type}/accuracy'].update_state(labels, probs)  # pyrefly: ignore[missing-attribute]
      metrics[f'{validation_type}/ece'].add_batch(probs, label=labels)
  ...
  for epoch in range(initial_epoch, FLAGS.train_epochs):
    ...
    logging.info(
        ...
        metrics['train/accuracy_no_pi'].result() * 100,  # pyrefly: ignore[unsupported-operation]
