...
"""ResNet-50 with TRAM layer on ImageNet-PI trained with maximum likelihood and gradient descent.
"""
...
flags.DEFINE_float(
    'no_pi_loss_weight',
    1.0,
    'Weight of the no-PI head loss in the TRAM objective.',
)
...
def main(argv):
  ...
  with strategy.scope():
    ...
    if FLAGS.use_heteroscedastic:
      logging.info('Building Keras ResNet-50 Het-TRAM model')
      model = ub.models.resnet50_het_tram(  # type: ignore
      ...
      model = ub.models.resnet50_tram(  # type: ignore
          input_shape=IMAGE_SHAPE,
          pi_input_shape=pi_shape,
          width_multiplier=FLAGS.model_width_multiplier,
          pi_tower_width=FLAGS.pi_tower_width,
  ...
  def train_step(iterator):
    ...
    def step_fn(inputs):
      ...
      with tf.GradientTape() as tape:
        ...
        loss = FLAGS.no_pi_loss_weight * negative_log_likelihood + negative_log_likelihood_pi + l2_loss
  ...
  def test_step(metrics_prefix, iterator, steps_per_eval):
    ...
    def step_fn(inputs):
      ...
      update_test_metrics(
          labels, logits, metric_prefix=metrics_prefix, metric_suffix='_no_pi')
      update_test_metrics(
          labels, logits_pi, metric_prefix=metrics_prefix, metric_suffix='_pi')
  ...
  for epoch in range(initial_epoch, FLAGS.train_epochs):
    ...
    logging.info(
        'Train Loss: %.4f, Accuracy (no PI): %.2f%%, Accuracy (PI): %.2f%%',
        metrics['train/loss'].result(),
        metrics['train/accuracy_no_pi'].result() * 100,
        metrics['train/accuracy_pi'].result() * 100)
    if FLAGS.train_proportion < 1.0:
      logging.info(
          'Validation NLL (no PI / noisy): %.4f, Accuracy (no PI / noisy): %.2f%%',
          metrics['validation_noisy/negative_log_likelihood_no_pi'].result(),
          metrics['validation_noisy/accuracy_no_pi'].result() * 100)
      logging.info(
      ...
      logging.info(
          'Validation NLL (no PI / clean): %.4f, Accuracy (no PI / clean): %.2f%%',
          metrics['validation_clean/negative_log_likelihood_no_pi'].result(),
          metrics['validation_clean/accuracy_no_pi'].result() * 100)
      logging.info(
    ...
    logging.info('Test NLL (no PI): %.4f, Accuracy (no PI): %.2f%%',
                 metrics['test/negative_log_likelihood_no_pi'].result(),
                 metrics['test/accuracy_no_pi'].result() * 100)
