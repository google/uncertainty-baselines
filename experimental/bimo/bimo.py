...
import uncertainty_baselines.experimental.bimo.svgd as svgd
import uncertainty_baselines.experimental.bimo.wide_resnet_bimo as wide_resnet_bimo
...
flags.DEFINE_float(
    ...
    'ensemble members.')
flags.DEFINE_float('svgd_temp', 1.0, 'the temperature for the SVGD update.')
flags.DEFINE_enum('svgd_method', 'weight', ['weight', 'functional'],
                  'SVGD method.')
flags.DEFINE_enum('model', 'bimo', ['bimo', 'mimobimo'],
                  'Model to use.')
flags.DEFINE_float('w_kern_bandwidth', 1.0,
                   'The bandwidth for the SVGD weight kernel.')
flags.DEFINE_float('f_kern_bandwidth', 1.0,
                   'The bandwidth for the SVGD function kernel.')
flags.DEFINE_float('prior_var', 1.0,
...
flags.DEFINE_bool('train_local', False, 'Whether to train locally on cpu')
...
flags.FLAGS.set_default('output_dir',
                        '/cns/el-d/home/dieterichl/cifar_bimo_test2')
FLAGS = flags.FLAGS
...
def main(argv):
  ...
  if not FLAGS.train_local:
    ...
    logging.info('Using local training strategy.')
  ...
  with strategy.scope():
    ...
    model = wide_resnet_bimo.make_model(
        input_shape=list(ds_info.features['image'].shape),
        depth=28,
  ...
  def log_joint(model, inputs, labels, training=True):
    logits, log_likelihood = svgd.categorical_log_prob(
        model, inputs, labels, training=training)
    log_prior = svgd.unnormalized_log_gaussian_prior(
        model, variance=FLAGS.prior_var)
    log_joints = log_likelihood + (log_prior / train_batch_size)
    return logits, log_likelihood, log_joints

  def grad_fn(images, labels):
    if FLAGS.svgd_method == 'weight':
      return svgd.neg_svgd_grad(
          model,
          log_joint,
          images,
          labels,
          temperature=FLAGS.svgd_temp,
          kern_bandwidth=FLAGS.w_kern_bandwidth,
          num_devices=strategy.num_replicas_in_sync)
    else:
      return svgd.neg_fsvgd_grad(
          model, log_joint,
          functools.partial(svgd.rbf_weight_kernel,
                            bandwidth=FLAGS.w_kern_bandwidth),
          functools.partial(svgd.rbf_function_kernel,
                            bandwidth=FLAGS.f_kern_bandwidth),
          images, labels,
          temperature=FLAGS.svgd_temp,
          num_devices=strategy.num_replicas_in_sync)
  ...
  def train_step(iterator):
    ...
    def step_fn(inputs):
      ...
      # log_probs is [num_members]
      # priors is [num_members]
      logits, likelihoods, log_probs, grads = grad_fn(images, labels)
      optimizer.apply_gradients(model.pair_grads_and_vars(grads))

      loss = -tf.reduce_mean(log_probs)
      # [ensemble_size * batch_size, num_classes]
      probs = tf.nn.softmax(tf.reshape(logits, [-1, num_classes]))
      # [ensemble_size, batch_size]
      tiled_labels = tf.tile(labels[tf.newaxis, :], [FLAGS.ensemble_size, 1])
      # [ensemble_size * batch_size]
      flat_labels = tf.reshape(tiled_labels, [-1])
      metrics['train/ece'].add_batch(probs, label=flat_labels)
      metrics['train/loss'].update_state(loss)
      metrics['train/negative_log_likelihood'].update_state(
          -tf.reduce_mean(likelihoods))
  ...
  def test_step(iterator, dataset_split, dataset_name, num_steps):
    ...
    def step_fn(inputs):
      ...
      images = inputs['features']
      labels = inputs['labels']
      # [num_ensemble_members, batch_size, num_classes]
      logits = model(images, training=False)
      # Permute the logits to make the rest of this code the same.
      # [batch_size, num_ensemble_members, num_classes]
      ...
      labels_tiled = tf.tile(labels[:, tf.newaxis], [1, FLAGS.ensemble_size])
