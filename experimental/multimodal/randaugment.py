...
def distort_image_with_randaugment(image, num_layers, magnitude, seed):
  ...
  for layer_num in range(num_layers):
    seed, op_seed = tf.unstack(tf.random.experimental.stateless_split(seed, 2))
    op_to_select = tf.random.stateless_uniform(
        [], op_seed, maxval=len(available_ops), dtype=tf.int32)
    random_magnitude = float(magnitude)
    with tf.name_scope('randaug_layer_{}'.format(layer_num)):
      for (i, op_name) in enumerate(available_ops):
        seed, prob_seed = tf.unstack(
            tf.random.experimental.stateless_split(seed, 2))
        prob = tf.random.stateless_uniform([], prob_seed, minval=0.2,
                                           maxval=0.8, dtype=tf.float32)
