...
flags.DEFINE_string('dataset_name', '', 'Name of registered dataset to use.')
...
flags.DEFINE_integer('num_rounds', 3, 'Number of rounds of active learning.')
...
def main(_) -> None:
  ...
  if FLAGS.train_main_only:
    ...
    base_model = model_class(
        train_bias=False, name='main_only_model', hidden_sizes=[1024, 512, 128])

    introspective_model = IntrospectiveActiveSampling(model=base_model,
                                                      train_bias=False,
                                                      name='main_only')

    introspective_model = compile_model(introspective_model, train_bias=False)
