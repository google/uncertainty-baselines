...
# DO NOT SUBMIT check 255
# ImageNet statistics. Used to normalize the input to Efficientnet. Note that
# these do NOT have `* 255.` after them like in the older ImageNet pipeline, as
...
class ImageNetDataset(base.BaseDataset):
  ...
  def __init__(
      ...
      preprocessing_type: str = 'inception',
  ...
  def _create_process_batch_fn(
    ...
    use_random_shuffling = self._mixup_params.get('use_random_shuffling', False)

    # DO NOT SUBMIT check that always want to run mixup on validation set
    if (self._is_training or self._is_validation) and mixup_alpha > 0.0:
      if self._mixup_params.get('adaptive_mixup', False):
        if 'mixup_coeff' not in self._mixup_params:
          # Hard target in the first epoch!
          if 'ensemble_size' not in self._mixup_params or 'num_classes' not in self._mixup_params:
            raise ValueError(
