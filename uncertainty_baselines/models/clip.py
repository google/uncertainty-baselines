...
https://github.com/google-research/scenic/blob/main/scenic/projects/baselines/clip/layers.py.

This version was adapted from
https://github.com/google-research/scenic/blob/main/scenic/projects/baselines/clip/layers.py.

[1] Alec Radford, Jong Wook Kim, et al.
...
class VisionTransformer(nn.Module):
  ...
  def __call__(self,
    ...
    x = Transformer(
        self.features, self.num_layers, self.num_heads, name='transformer')(x)
...
class CLIP(nn.Module):
  ...
  def encode_image(self,
    ...
    if scale_logits:
      ...
      # in the Scenic implementation.
      # jnp.dot(sqrt(t)*zimg, sqrt(t)*ztxt.T)  = jnp.dot(zimg, ztxt.T) * t
  ...
  def __call__(self,
    ...
    if image is not None:
      x = self.encode_image(image, normalize, scale_logits)
    if text is not None:
      y = self.encode_text(text, normalize, scale_logits)
    return x, y
...
def clip(
  ...
  """Builds a CLIP (Contrastive Language-Image Pre-Training) model.

  The model consists of a vision and a text component. The vision component is
  a ResNet or a Vision Transformer. The text component is a text Transformer.
