...
class Radial(distribution.Distribution):
  r"""
  Creates a Radial distribution as described in
  Radial Bayesian Neural Networks (Farquhar et al., AISTATS 2020).
  ...
  def __init__(
      self, loc, presoftplus_scale, batch_shape=tuple(),
      dtype=tf.float32, validate_args=False,
      allow_nan_stats=True, name='Radial'):
    """
    Args:
      loc: `Tensor` representing the mean of the distribution.
      presoftplus_scale: `Tensor` representing the pre-softplus scale,
        `\rho`.
      batch_shape: Positive `int`-like vector-shaped `Tensor` representing
        the new shape of the batch dimensions. Default value: [].
      validate_args: Python `bool`, default `False`.
        When `True`, distribution parameters are checked for
          validity despite possibly degrading runtime performance.
        When `False` invalid inputs may silently render incorrect
          outputs.
      allow_nan_stats: Python `bool`, default `True`.
        When `True`, statistics (e.g., mean, mode, variance) use the
          value "`NaN`" to indicate the result is undefined.
        When `False`, an exception is raised if one or more of the
    ...
    with tf.name_scope(name) as name:
      shape_dtype = dtype_util.common_dtype(
      ...
      super(Radial, self).__init__(
        dtype=dtype, validate_args=validate_args,
  ...
  def log_prob(self, x):
    ...
    :math:`q(w^x) = \frac{q(\epsilon^r)}{\prod_k \sigma_k^x|\frac{\partial\epsilon_i^x}{\partial\epsilon_j^r}|^{-1}`

    and we have:
    :math:`|\frac{\partial\epsilon_i^x}{\partial\epsilon_j^r}| = (\epsilon_0^r)^{D-1}\prod_{i=2}^{D}(\sin(\epsilon_i^r))^{i-1}`

    and equation 26:
    :math:`q(\epsilon^r)=\frac{1}{\sqrt{2\pi}}e^{-\frac{\epsilon_0^2}{2}}\prod_{i=1}^{D-1}\sin(\epsilon_i^r)^{D-i}`

    Args:
        x (Tensor): must be of shape:
    ...
    epsilon_cartesian_nonevent_shape = epsilon_cartesian_shape[
                                       :-len(self.event_shape)]
    epsilon_cartesian = tf.reshape(
      epsilon_cartesian, tuple(epsilon_cartesian_nonevent_shape) + (-1,))
    ...
    jacobian = epsilon_radial[:, 0] ** (dimension - 1) * tf.math.prod(
      tf.math.pow(tf.math.sin(epsilon_radial[..., 1:]),
                  tf.math.arange(1, dimension)), axis=-1)

    # q(epsilon^r)
    noise_density = (
        (1 / tf.math.sqrt(2 * tf.constant(math.pi)))
        * math.exp(-0.5 * epsilon_radial[..., 0] ** 2)
        * tf.math.prod(
      tf.math.pow(tf.math.sin(epsilon_radial[..., 1:]),
                  dimension - tf.arange(1, dimension)), axis=-1))
  ...
  def _cartesian_to_hyperspherical(self, x_c):
    ...
    Args:
      x_c: `Tensor` input in cartesian coordinates (weight space).
        Has shape sample_shape + batch_shape + flattened_event_shape.
    Returns:
    ...
    theta = tf.math.atan(base[..., 1:] / x_c[..., 0:-1]) + (tf.constant(
      math.pi) / 2)  # The first element is x_0 / base_1

    # Correcting the last element
    theta[..., -1] = tf.math.atan(x_c[..., -1] /
                                  (x_c[..., -2] + tf.math.sqrt(
                                    (x_c[..., -1] ** 2) + (
                                        x_c[..., -2] ** 2)))) + (
  ...
  def _sample_n(self, n, seed=None):
    ...
    distance = samplers.normal(
      shape=tf.concat(
        [[n], self.batch_shape, [1] * len(self.event_shape)], axis=0),
      seed=seed, dtype=self.dtype)
  ...
  def kl_divergence(self, other, name='kl_divergence'):
    if (isinstance(other, distributions.independent.Independent) and
...
def kl_radial_normal(p, q, n_samples=10, name=None):
  with tf.name_scope(name or 'kl_radial_normal'):
    if p.event_shape != q.event_shape:
      raise ValueError(
        f'KL-divergence between Radial and Multivariate Normals with'
...
class TrainableRadial(tf.keras.layers.Layer):
  ...
  def __call__(self, shape, dtype=None):
    ...
    return generated_random_variables.make_random_variable(Radial)(
      loc=mean,
      presoftplus_scale=tf.math.log(tf.math.expm1(stddev)))
...
class TrainableHeRadial(TrainableRadial):
  """Trainable radial initialized per He et al. 2015, given a ReLU nonlinearity.
