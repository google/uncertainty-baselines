...
def resnet50_heteroscedastic_resizable_width(
    input_shape,
    num_classes,
    temperature,
    num_factors,
    ...
    width_multiplier=1.):
  """Builds ResNet50 with heteroscedastic output and resizable width.
  ...
  Args:
    ...
    temperature: Float or scalar `Tensor` representing the softmax temperature.
    num_factors: Integer. Number of factors to use in approximation to full rank
      covariance matrix. If num_factors <= 0, then the diagonal covariance
    ...
    eps: Float. Clip probabilities into [eps, 1.0] softmax or [eps, 1.0 - eps]
      sigmoid before applying log (softmax), or inverse sigmoid.
    return_unaveraged_logits: Boolean. Whether to also return the logits before
      taking the MC average over samples.
    tune_temperature: Boolean. If True, the temperature is optimized during the
      training as any other parameters.
