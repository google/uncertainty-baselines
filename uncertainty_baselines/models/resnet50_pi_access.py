...
def resnet50_pi_access(input_shape,
                       pi_input_shape,
                       num_classes,
                       width_multiplier=1.,
                       omit_last_layer=False,
                       pi_tower_width=1024,
                       name='resnet50_pi_access'):
  """Builds width-resized ResNet50 with access to privileged information.
  ...
  logits = logits_pi_fc(joint_feature_space)  # pyrefly: ignore[not-callable]

  return tf.keras.Model(inputs=(inputs, pi_inputs), outputs=logits, name=name)
