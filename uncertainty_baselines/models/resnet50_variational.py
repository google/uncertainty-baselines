...
def bottleneck_block(
    inputs, filters, stage, block, strides, prior_stddev,
...
def group(
    inputs, filters, num_blocks, stage, strides, prior_stddev,
    dataset_size, stddev_init):
  blocks = string.ascii_lowercase
  x = bottleneck_block(
    inputs, filters, stage, block=blocks[0], strides=strides,
    prior_stddev=prior_stddev, dataset_size=dataset_size,
    stddev_init=stddev_init)
  for i in range(num_blocks - 1):
    x = bottleneck_block(
      x, filters, stage, block=blocks[i + 1], strides=1,
      prior_stddev=prior_stddev, dataset_size=dataset_size,
      stddev_init=stddev_init)
  return x
...
def resnet50_variational(
    input_shape, num_classes, prior_stddev, dataset_size, stddev_init,
  ...
  x = group(
    x, [64, 64, 256], stage=2, num_blocks=3, strides=1,
    prior_stddev=prior_stddev, dataset_size=dataset_size,
    stddev_init=stddev_init)
  x = group(
    x, [128, 128, 512], stage=3, num_blocks=4, strides=2,
    prior_stddev=prior_stddev, dataset_size=dataset_size,
    stddev_init=stddev_init)
  x = group(
    x, [256, 256, 1024], stage=4, num_blocks=6, strides=2,
    prior_stddev=prior_stddev, dataset_size=dataset_size,
    stddev_init=stddev_init)
  x = group(
    x, [512, 512, 2048], stage=5, num_blocks=3, strides=2,
    prior_stddev=prior_stddev, dataset_size=dataset_size,
    stddev_init=stddev_init)
