...
def init_kernel_regularizer(kernel_regularizer_class, dataset_size,
                            prior_stddev, inputs, n_filters=None,
                            kernel_size=None, n_outputs=None,
                            prior_stddev_scale=2., prior_stddev_mode='fan_in'):
  """
  ...
  Args:
    ...
    kernel_size: int, we assume that each channel of the 2D
      convolutional kernel has shape (kernel_size, kernel_size).
    n_outputs: int, number of outputs of the layer if it is Dense.
    prior_stddev: float, a fixed stddev for the prior. If it is not supplied, we
      compute a prior_stddev dependent on input size, defaulting to the stddev
      computation used in He initialization.
  ...
  return kernel_regularizer_class(
    stddev=stddev, scale_factor=1. / dataset_size)


def get_2d_conv_kernel_shape(inputs, n_filters, kernel_size):
  """
  Compute the shape of a 2D convolutional kernel using the input size,
  number of filters, and kernel size. This can be used to provide a
  shape-dependent value at initialization of the Layer itself.
