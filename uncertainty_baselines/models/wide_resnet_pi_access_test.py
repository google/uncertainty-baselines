...
class WideResnetPIAccessTest(tf.test.TestCase):

  def testWideResnetPIAccess(self):
    ...
    model = ub.models.wide_resnet_pi_access(
        ...
        l2=0.,
        version=2)
    model.compile('adam', loss=_avg_annotator_loss)
    history = model.fit(
