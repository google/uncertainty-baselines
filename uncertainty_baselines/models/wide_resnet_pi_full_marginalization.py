...
"""Wide Residual Network with PI access and approximate full marginalization output head."""
...
def wide_resnet_pi_full_marginalization(
  ...
  """Builds Wide ResNet with a PI access and approximate full marginalization head.
  ...
  Args:
    ...
    num_mc_samples: Number of Monte-Carlo samples used to perform approximate
      full marginalization of the PI.
  ...
  pi_mc_samples = tf.keras.Input(
      shape=(
          num_mc_samples,
          pi_input_shape[0],  # type: ignore
          pi_input_shape[1]),  # type: ignore
      name='pi_mc_samples')
  ...
  pi_fc = tf.keras.layers.Dense(
      pi_tower_width, activation='relu', name='pi_embedding')
  ...
  probs_fm = tf.reduce_mean(probs_fm, 1)

  return tf.keras.Model(
      inputs=(inputs, pi_inputs, pi_mc_samples),
      outputs=(logits_pi, tf.stop_gradient(probs_fm)),
      name='wide_resnet-{}-{}-{}-pi-fm'.format(depth, width_multiplier,
                                               pi_tower_width))
