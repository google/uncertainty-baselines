...
"""Tests for Wide ResNet with TRAM head."""

import tensorflow as tf
import uncertainty_baselines as ub


class WideResnetTRAMTest(tf.test.TestCase):

  def testWideResnetTRAM(self):
    ...
    dataset = tf.data.Dataset.from_tensor_slices(
        ((features, pi_features), labels))
    dataset = dataset.repeat().shuffle(dataset_size).batch(batch_size)

    def _avg_annotator_loss(labels, logits):
    ...
    model = ub.models.wide_resnet_tram(
        ...
        l2=0.,
        version=2)
    model.compile('adam', loss=(_avg_annotator_loss, _avg_annotator_loss))
    history = model.fit(
